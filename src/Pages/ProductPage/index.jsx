...
const ProductPage = () => {
    ...
    const currentProducts = products[0];
    ...
    const slide = (direction) => {
        var slider = document.getElementById(`slider-${currentProducts.id}`);
    ...
    const handleDeleteProduct = async () => {
        try {
            ...
            const res = await fetch(`/api/Product/DeleteProduct/${currentProducts.id}`, {
                method: "DELETE",
            });
                ...
                <div className=" py-2 border-b border-[#ADADAD] dark:border-[#464646]">
                    ...
                    <div className="flex justify-between gap-2">
                        <div className="flex">
                            ...
                            <Link to={`/${user.username}`}>
                                <p
                                    className="font-semibold cursor-pointer hover:underline transition-[0.5] dark:text-[#FAFAFA]"
                                    onClick={(e) => {
                                        e.preventDefault();
                                ...
                                >
                                    ...
                                    <span className="bg-[#864eff] text-sm font-semibold px-2 text-[#FFFFFF] rounded-full">{currentProducts.category}</span>
                                </p>
                            </ Link>
                        </div>
                        <div className="flex justify-between gap-2">
                            <p className="text-md text-[#646464]">
                                {formatDistanceToNow(new Date(currentProducts.createdAt), { locale: es })}
                            </p>
                    ...
                    {/* product body */}
                    <div className='relative flex items-center my-2 w-full mb-2 overflow-hidden'>
                        ...
                        >
                            <ChevronLeft className='opacity-50 hover:opacity-100 hover:opacity-100 dark:hover:opacity-20'
                                size={40}
                            />
                        </div>
                        <div
                            id={`slider-${currentProducts.id}`}
                            className='w-full h-full overflow-x-scroll scroll whitespace-nowrap scroll-smooth scrollbar-hide'
                        >
                            {currentProducts.imgs.map((img, index) => (
                        ...
                        >
                            <ChevronRight className='opacity-50 hover:opacity-100 hover:opacity-100 dark:hover:opacity-20'
                                size={40}
                            />
                        </div>
                    </div>
                    ...
                    <div className="my-2 ml-10">
                        <p className="text-2xl font-bold text-[#303030] dark:text-[#FAFAFA] mb-1">{currentProducts.title}</p>
                        <p className="text-xl font-semibold text-[#303030] dark:text-[#FAFAFA] mb-2">${currentProducts.price}</p>

                        {currentProducts.detailedDescription.split('\n').map((line, index) => (
                    ...
                    <div className="ml-10">
                        ...
                        <ActionsProduct produ={currentProducts} />
                    </div>
                </div>

                <div className="mt-2 pb-2">
                    {currentProducts.replies.map((reply, index) => (
